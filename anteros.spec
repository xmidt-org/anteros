...
License:    ASL 2.0
URL:        https://github.com/Comcast/anteros
...
%{__install} -p src/anteros/%{name} %{buildroot}%{_bindir}
...
%files
%defattr(644, anteros, anteros, 755)

# Binary
%dir %{_bindir}
%attr(755, anteros, anteros) %{_bindir}/%{name} 

# Init.d
%attr(755, anteros, anteros) %{_initddir}/%{name}

# Configuration
%dir %{_sysconfdir}/%{name}
%config %attr(644, anteros, anteros) %{_sysconfdir}/%{name}/%{name}.env.example
%config %attr(644, anteros, anteros) %{_sysconfdir}/%{name}/%{name}.json
%config %attr(644, anteros, anteros) %{_sysconfdir}/%{name}/supervisord.conf
